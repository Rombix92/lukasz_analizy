# -*- coding: utf-8 -*-
"""
Created on Wed Oct 23 14:15:17 2019
...
###############Prediction of being republican/democrat based on voiting
...
df=df.replace(to_replace =["?","n","y"], value = [np.nan,0,1])
df=df.dropna().reset_index(drop=True)
#features - scikitlearn practice
X=df.drop("party", axis=1)
#target - scikitlearn practice
Y=df["party"].replace(to_replace =["democrat","republican"], value = [0,1])
...
#stratify=y  stratifies the split according to the labels so that they are distributed in the training and test sets as they are in the original dataset.
...
##########################predicting life time based on fertility
...
#####Using cross validation to compute R^2 independent from spliting luck
...
%timeit cross_val_score(reg, X_test,y_test,cv=3)

############Using Lasso  feature selection 
#Lasso parameter selection characterize with choosing most important predictors, while shrinking the less important predicotors coefficients to 0
...
df=pd.read_csv(url, header=0)
df=df[(df["age"]!=0) & (df["dpf"]!=0) & (df["bmi"]!=0) & (df["insulin"]!=0) & (df["triceps"]!=0) & (df["diastolic"]!=0)]
...
##############Logistic Regression for the same diabetes database
#logistic regression provide linear decision boundary
...
#calculatin auc with cross validation
# Import necessary modules
from sklearn.model_selection import cross_val_score
...
###################Choosing best parameter C for logistic regression
...
#I will now practice evaluating a model with tuned hyperparameters on a hold-out set
...
# and tuning hyperparameteres on a hold_out_dataset on linear regression
...
#I'll now practice setting up a pipeline with two steps: the imputation step, followed by the instantiation of a classifier.
